...
class Augmentation:
    ...
    def random_mosaic(self, images, boxes, target_img_size):
        ...
        for i, img in enumerate(images):
            ...
            if boxes[i].size:
                ...
                new_area = (new_boxes[:, 2] - new_boxes[:, 0]) * (
                    new_boxes[:, 3] - new_boxes[:, 1]
                )
                origin_area = (boxes[i][:, 2] - boxes[i][:, 0]) * (
                    boxes[i][:, 3] - boxes[i][:, 1]
                )
                new_boxes = new_boxes[
                    (new_area / origin_area) >= self.mosaic_min_area_percent, :
                ]
    ...
    def random_perspective(self, im, boxes=(), border=(0, 0)):
        """Random mapping transformation, this step includes translation,
        perspective transformation, rotation and scaling, miscut, translation"""
        # targets = [cls, xyxy]
        ...
        # target bounding box follow transform
        n = len(boxes)
        if n:
            xy = np.ones((n * 4, 3))
    ...
    def random_hsv(self, im):
        ...
        if self.hsv_h or self.hsv_s or self.hsv_v:
            r = (
                np.random.uniform(-1, 1, 3) * [self.hsv_h, self.hsv_s, self.hsv_v] + 1
            )  # random gains
...
if __name__ == "__main__":
    ...
    im3, box3 = ag.random_mosaic(
        images=[im, im, im, im], boxes=([box, box, box, box]), target_img_size=320
    )
    ...
    for i, b in enumerate(box5):
        im5 = draw_bounding_box(
            im5, "dog", 1, x_min=b[0], y_min=b[1], x_max=b[2], y_max=b[3]
        )

...
def save_coco_all_wh(anno_file, output_anchor_file):
    """生成coco标注数据目标边框宽高
    :param anno_file:
    :param image_shape:
    ...
    coco = CoCoDataGenrator(
        coco_annotation_file=anno_file,
        img_shape=(512, 512, 3),
...
def compute_iou(x, centers):
    ...
    # 1. x,centers 复制
    box1 = np.reshape(
        np.tile(np.expand_dims(x, axis=1), [1, 1, np.shape(centers)[0]]), [-1, 2]
    )
    xw, xh = box1[:, 0], box1[:, 1]
    ...
    while True:
        ...
        if np.all(max_center_ids == pre_max_center_ids):
            if save_cluster_fig:
                for i in range(K):
                    plt.scatter(
                        x[max_center_ids == i, 0],
                        x[max_center_ids == i, 1],
                        label=i,
                        s=10,
                    )
...
if __name__ == "__main__":
    save_coco_all_wh(
        anno_file="../newDataset/train/_annotations.coco.json",
        output_anchor_file="../newDataset/train/_val_all_wh.txt",
    )
    # ../dataset/valid/_annotations.coco.json
    data = get_wh("../newDataset/train/_val_all_wh.txt")
    kmeans(data, 12, save_cluster_fig="./anchors_320x320_test.png")

...
class ComputeLoss:
    ...
    def _transform_expand_target(self, gt_box_class_anchor, grid_size):
        ...
        y_true_out = np.zeros(
            (batch, grid_size, grid_size, len(self.anchor_masks[0]), 6),
            dtype=np.float32,
        )
                ...
                if w_ratio_bool and h_ratio_bool:
                    ...
                    if self.only_best_anchor:
                        y_true_out[i, grid_xy[0], grid_xy[1], best_anchor_id, :] = [
                            ...
                            box[3],
                            1,
                            gt_box_class_anchor[i, j, 4],
                        ]
                    else:
                        y_true_out[i, grid_xy[0], grid_xy[1], :, :] = np.array(
                            [
                                [
                                    box[0],
                                    box[1],
                    ...
                    xy = box_xy * grid_size
                    inv_xy = grid_size - xy
                    if (xy[0] % 1 < 0.5) and (xy[0] > 1.0):
                        jxy = xy - np.array([0.5, 0])
                        grid_jxy = np.array(jxy, np.int32)
                        if self.only_best_anchor:
                            y_true_out[
                                i, grid_jxy[0], grid_jxy[1], best_anchor_id, :
                            ] = [
                                box[0],
                                box[1],
                                box[2],
                            ...
                            y_true_out[i, grid_jxy[0], grid_jxy[1], :, :] = np.array(
                                [
                                    [
                                        box[0],
                                        box[1],
                    ...
                    if (xy[1] % 1 < 0.5) and (xy[1] > 1.0):
                        kxy = xy - np.array([0, 0.5])
                        grid_kxy = np.array(kxy, np.int32)
                        if self.only_best_anchor:
                            y_true_out[
                                i, grid_kxy[0], grid_kxy[1], best_anchor_id, :
                            ] = [
                                box[0],
                                box[1],
                                box[2],
                            ...
                            y_true_out[i, grid_kxy[0], grid_kxy[1], :, :] = np.array(
                                [
                                    [
                                        box[0],
                                        box[1],
                    ...
                    if (inv_xy[0] % 1 < 0.5) and (inv_xy[0] > 1.0):
                        inv_lxy = xy + np.array([0.5, 0])
                        grid_lxy = np.array(inv_lxy, np.int32)
                        if self.only_best_anchor:
                            y_true_out[
                                i, grid_lxy[0], grid_lxy[1], best_anchor_id, :
                            ] = [
                                box[0],
                                box[1],
                                box[2],
                            ...
                            y_true_out[i, grid_lxy[0], grid_lxy[1], :, :] = np.array(
                                [
                                    [
                                        box[0],
                                        box[1],
                    ...
                    if (inv_xy[1] % 1 < 0.5) and (inv_xy[1] > 1.0):
                        inv_mxy = xy + np.array([0, 0.5])
                        grid_mxy = np.array(inv_mxy, np.int32)
                        if self.only_best_anchor:
                            y_true_out[
                                i, grid_mxy[0], grid_mxy[1], best_anchor_id, :
                            ] = [
                                box[0],
                                box[1],
                                box[2],
                            ...
                            y_true_out[i, grid_mxy[0], grid_mxy[1], :, :] = np.array(
                                [
                                    [
                                        box[0],
                                        box[1],
    ...
    def build_targets(self, predicts, gt_boxes, gt_classes):
        ...
        # [batch, num_box, 3, (w, h)]
        box_wh = np.tile(
            np.expand_dims(box_wh, axis=-2), (1, 1, len(self.anchor_masks[0]), 1)
        )
        # [batch, num_box, 3]
        ...
        for i, predict in enumerate(predicts):
            ...
            intersection = np.minimum(box_wh[..., 0], cur_anchors[..., 0]) * np.minimum(
                box_wh[..., 1], cur_anchors[..., 1]
            )
            iou = intersection / (box_area + anchor_area - intersection)
            anchor_idx = np.array(
                cur_anchor_ids[np.argmax(iou, axis=-1)], dtype=np.float32
            )
            anchor_idx = np.expand_dims(anchor_idx, axis=-1)

            # 拼接最后的结果
            # [batch, num_box, (x1, y1, x2, y2, class, best_anchor_id)]
            gt_box_class_anchor = np.concatenate(
                [gt_boxes, gt_classes, anchor_idx], axis=-1
            )
    ...
    def __call__(self, predicts, gt_boxes, gt_classes):
        ...
        for i, predict in enumerate(predicts):
            ...
            pred_xy, pred_wh, pred_obj, pred_cls = tf.split(
                predict, (2, 2, 1, self.num_class), axis=-1
            )

            # [batch, grid, grid, anchors, 2]
            pred_xy = 2 * tf.sigmoid(pred_xy) - 0.5
            # [batch, grid, grid, anchors, 2]
            pred_wh = (tf.sigmoid(pred_wh) * 2) ** 2 * self.anchors[
                self.anchor_masks[i]
            ]
            ...
            pred_grid_xy = (pred_xy + tf.cast(grid, tf.float32)) / tf.cast(
            ...
            x1 = tf.minimum(tf.maximum(x1, 0.0), self.image_shape[1])
            y1 = tf.minimum(tf.maximum(y1, 0.0), self.image_shape[0])
            x2 = tf.minimum(tf.maximum(x2, 0.0), self.image_shape[1])
            y2 = tf.minimum(tf.maximum(y2, 0.0), self.image_shape[0])
            ...
            iou = bbox_iou(
                tf.boolean_mask(pred_box, obj_mask > 0),
                tf.boolean_mask(true_box, obj_mask > 0),
                CIoU=True,
            )
            box_loss = 1.0 - iou
